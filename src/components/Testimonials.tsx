...
function TestimonialCard({ t, delay }: { t: typeof testimonials[0]; delay: number }) {
  ...
  return (
    <div ref={ref}
      style={{ background: "white", borderRadius: "1.75rem", padding: "2rem", border: "1px solid #EDE5E3", opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(28px)", transition: `opacity .7s ease ${delay}ms, transform .7s ease ${delay}ms` }}
      ...
      <p style={{ fontSize: "0.95rem", color: "#1C1010", lineHeight: 1.75, marginBottom: "1.5rem", fontStyle: "italic", fontWeight: 500 }}>
        &ldquo;{t.text}&rdquo;
      </p>
...
export default function Testimonials() {
  const { ref, visible } = useReveal();
  return (
    <section id="testimonials" style={{ background: "#FAF7F5", padding: "7rem 1.5rem" }}>
      <div style={{ maxWidth: 1160, margin: "0 auto" }}>
        <div ref={ref} className="reveal" style={{ textAlign: "center", ...(visible ? { opacity:1, transform:"none" } : {}) }}>
          ...
          <h2 style={{ fontWeight: 800, fontSize: "clamp(1.8rem,3.5vw,2.8rem)", color: "#1C1010", marginTop: "0.7rem", letterSpacing: "-.02em" }}>
            Des jeunes qui ont testé,<br /><span style={{ color: P }}>et qui restent</span>
          </h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5" style={{ marginTop: "3.5rem" }}>
